...
// GetMyWishes handles GET /api/wishes/mine
...
func GetMyWishes(c *gin.Context, db *gorm.DB) {
	...
	// 2. 解析分页参数（使用前端约定的 page 和 pageSize）
	...
	// 7. 返回成功响应（响应中使用 page 和 pageSize 字段以匹配前端约定）
	c.JSON(http.StatusOK, gin.H{
		"code":    apperr.SUCCESS,
