...
├── cmd/myapp/main.go # Go 应用主入口 (包含 seeder 调用) ├── internal/ │ ├── app/ │ │ ├── handler/ # HTTP 处理器 (业务逻辑) │ │ ├── model/ # GORM 数据库模型 │ │ └── service/ # 第三方服务 (如 ai_service.go) │ ├── middleware/ # Gin 中G间件 (如 auth.go) │ ├── pkg/ # 内部公共包 (database, logger, jwt, err) │ └── router/ # 路由配置 (router.go) ├── nginx/ │ ├── Dockerfile # Nginx 镜像配置文件 │ └── nginx.conf # Nginx 反向代理配置 ├── .env # (需手动创建) 环境变量，包含所有密钥 ├── .gitignore # Git 忽略配置 ├── docker-compose.yml # Docker 编排文件 (Nginx, Go App, MySQL) ├── Dockerfile # Go 应用的 Docker 镜像配置文件 (多阶段构建) ├── go.mod # Go 模块依赖 └── README.md # 你正在阅读的文档
